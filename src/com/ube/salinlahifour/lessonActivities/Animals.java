...
public class Animals extends AbstractLessonActivity implements OnClickListener {
	...
	protected boolean checkAnswer(String answer) {
		...
		Log.d("English: " + englishCards.get(englishCardIndex).itemno + " Sound: " + soundCards.get(soundCardIndex).itemno + " Picture: " + pictureCards.get(pictureCardIndex).itemno + " Filipino: " + filipinoCards.get(filipinoCardIndex).itemno, "TEST");
		...
		if(fixedIndex == 0) {
			...
			if(btn_down[3].getVisibility() == View.VISIBLE) { // Evaluate only
																// if it is not
																// yet checked
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.btn_picturecard:
				// sound =
				// MediaPlayer.create(this,pictureCards.get(pictureCardIndex).soundResID);
				// sound.start();
				break;
